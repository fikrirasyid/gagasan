...
if( function_exists( 'get_the_subtitle' ) ) :
	 ...
	 */
	if( ! function_exists( 'gagasan_subtitles_remove_default' ) ) :
	function gagasan_subtitles_remove_default( $subtitle_view_supported ){
	...
	add_filter( 'subtitle_view_supported', 'gagasan_subtitles_remove_default' );
	 ...
	 */
	if( ! function_exists( 'gagasan_subtitles_implementation' ) ) :
	function gagasan_subtitles_implementation( $the_categories, $post_id ){
	...
	add_filter( 'gagasan_entry_subtitle', 'gagasan_subtitles_implementation', 10, 2 );
	 ...
	 */
	if( ! function_exists( 'gagasan_subtitles_posted_on' ) ) :
	function gagasan_subtitles_posted_on( $posted_on, $post_id ){
		...
		if( $subtitle && is_singular() ){
			...
			$entry_subtitle = gagasan_get_entry_subtitle( $post_id );

			if( $entry_subtitle ){

				return sprintf( __( '%s under <span class="taxonomies">%s</span>', 'gagasan' ), $posted_on, $entry_subtitle );
	...
	add_filter( 'gagasan_posted_on', 'gagasan_subtitles_posted_on', 10, 2 );
